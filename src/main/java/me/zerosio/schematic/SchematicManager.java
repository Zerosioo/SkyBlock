...
public class SchematicManager {
    ...
    public static void pasteSchematic(String name, Location pasteAt, int rotation) throws IOException {
        ...
        WorldServer world = ((CraftWorld) pasteAt.getWorld()).getHandle();
        ...
        for (SchematicBlock block : blocks) {
            ...
            Chunk chunk = world.getChunkAtWorldCoords(new BlockPosition(finalX, finalY, finalZ));
        ...
        for (org.bukkit.entity.Player player : pasteAt.getWorld().getPlayers()) {
            EntityPlayer nmsPlayer = ((CraftPlayer) player).getHandle();
            for (int cx = cx1; cx <= cx2; cx++) {
                for (int cz = cz1; cz <= cz2; cz++) {
                    PacketPlayOutMapChunk packet = new PacketPlayOutMapChunk(world.getChunkAt(cx, cz), true, 65535);
