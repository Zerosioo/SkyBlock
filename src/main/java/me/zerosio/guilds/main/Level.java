...
public enum Level {
    ...
    private static final int NERF_MULTIPLIER = 22;
    private final long experienceNeeded;

    Level(long rawExperienceNeeded) {
        this.experienceNeeded = rawExperienceNeeded * NERF_MULTIPLIER;
    }
    ...
    public static int getLevelFromXP(long xp) {
        ...
        for (Level level : Level.values()) {
            if (level.getExperienceNeeded() <= xp && level.getExperienceNeeded() > result.getExperienceNeeded()) {
                result = level;
            }
        }
        return result.ordinal() + 1;
    }
    ...
    public static Level getNextLevel(long xp) {
        Level current = getLevelFromLongXP(xp);
        int nextOrdinal = current.ordinal() + 1;
        if (nextOrdinal >= Level.values().length) {
            return current; // already max level
        }
    ...
    public Level getPastLevel() {
        ...
        return Level.values()[prevOrdinal];
    }
}
